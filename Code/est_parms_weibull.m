...
function [Calibrated_Model] = est_parms_weibull(x, name)
    ...
    SE = sqrt(diag(inv(hessian(@(z)weibull_ll(z,x),[theta_hat]))));
    t = [1:size(x,1)]'-1;
    w = theta_hat(2)*exp(-t*theta_hat(1));
    Calibrated_Model = struct('Model', name,'lambda', theta_hat(1),'eta_zero', theta_hat(2), 'SE', SE, 'w', w,  'LL',-Fval,'Problem', problem);
