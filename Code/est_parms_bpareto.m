...
if max(isempty(dmax),isempty(dmin)) == 0
    ...
    SE = sqrt(diag(inv(hessian(@(z)bpareto_ll(z,x,dmax,dmin),[theta_hat]))));
    Calibrated_Model = struct('Model', name,'lambda', theta_hat(1),'eta_zero', theta_hat(2), 'SE', SE, 'alpha', alpha, 'dmin',dmin, 'dmax', dmax, 'LL',-Fval, 'Estimated_Bounds',0,'Problem', problem);
    ...
    [theta_hat,Fval] = run(ms,problem,1000);
    alpha = 1./(theta_hat(2)*exp(-t*theta_hat(1)));
    SE = sqrt(diag(inv(hessian(@(z)bpareto_ll_v2(z,x),[theta_hat]))));
    Calibrated_Model = struct('Model', name,'lambda', theta_hat(1),'eta_zero', theta_hat(2), 'SE', SE, 'alpha', alpha, 'dmin',theta_hat(4), 'dmax', theta_hat(3), 'LL',-Fval, 'Estimated_Bounds',1,'Problem', problem);
end
