...
%     problem = createOptimProblem('fminunc', 'objective',@(y)weibull_ll(y,x), 'x0',[0.1,.9], 'options', options);
    ...
    [SE,HESS] = se_weibull(theta_hat,x);
    Calibrated_Model = struct('Model', name,'lambda', theta_hat(1),'eta_zero', theta_hat(2), 'w', w,  'LL',-Fval,'Problem', problem, 'SE', SE, 'VarCovar', HESS);
